...
func buildInlineResults(results []search.MediaResult, lang string, downloadURLs map[string]string) []tg.InlineQueryResult {
	out := make([]tg.InlineQueryResult, 0, len(results))
	for _, r := range results {
		...
		case search.CatImage:
			...
			out = append(out, tg.InlineQueryResultPhoto{
				ID:           r.ID,
				PhotoURL:     r.MediaURL,
...
// videoButtons builds the video card keyboard: an "Open on <platform>" link
...
func videoButtons(r search.MediaResult, lang string, download downloadButton) *tg.InlineKeyboardMarkup {
...
func promptResult(lang, botUsername string) []tg.InlineQueryResult {
